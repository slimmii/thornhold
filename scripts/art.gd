...
static var stone_texture: ImageTexture

static func stone(hex: String) -> StandardMaterial3D:
	var key = "stone_" + hex
	...
	if stone_texture == null:
		...
		for y in range(128):
			for x in range(128):
				var shade = 0.78 + noise.get_noise_2d(x, y) * 0.22
				pixels.set_pixel(x, y, Color(shade, shade, shade))
		pixels.generate_mipmaps()
		stone_texture = ImageTexture.create_from_image(pixels)
	var material = mat(hex).duplicate()
...
static func mat(hex: String, metal: float = 0.0, glow: float = 0.0) -> StandardMaterial3D:
	var key = hex + str(metal) + str(glow)
	if materials.has(key):
		return materials[key]
	var m = StandardMaterial3D.new()
	m.albedo_color = Color(hex)
	m.roughness = 0.87 if metal == 0 else 0.42
	m.metallic = metal
	if glow > 0:
...
static func light(parent: Node3D, pos: Vector3, color: Color, energy: float, reach: float) -> OmniLight3D:
	var node = OmniLight3D.new()
	node.position = pos
	node.light_color = color
	node.light_energy = energy
	node.omni_range = reach
	node.omni_attenuation = 1.5
	parent.add_child(node)
	return node
...
static func torch(parent: Node3D, pos: Vector3) -> Node3D:
	...
	node.position = pos
	parent.add_child(node)
	cylinder(node, Vector3(0, 0.6, 0), 0.11, 0.075, 1.2, mat("393339", 0.6))
	cylinder(node, Vector3(0, 1.25, 0), 0.12, 0.24, 0.27, mat("645646", 0.5))
	var flame = sphere(node, Vector3(0, 1.57, 0), 0.20, mat("ff8f27", 0, 0.8))
	flame.scale = Vector3(0.7, 1.9, 0.7)
	var inner = sphere(node, Vector3(0, 1.56, 0), 0.13, mat("ffd47b", 0, 0.9))
	inner.scale.y = 1.8
	light(node, Vector3(0, 1.8, 0), Color("ffb45d"), 2.1, 7.2)
	return node
...
static func banner(parent: Node3D, pos: Vector3, yaw: float = 0.0) -> void:
	...
	box(node, Vector3(0, 1.76, 0), Vector3(1.1, 0.075, 0.12), mat("c8ac70", 0.6))
	box(node, Vector3(-0.37, 0.8, -0.03), Vector3(0.035, 1.72, 0.015), mat("bca374"))
	box(node, Vector3(0.37, 0.8, -0.03), Vector3(0.035, 1.72, 0.015), mat("bca374"))
	var emblem = box(node, Vector3(0, 0.9, -0.04), Vector3(0.26, 0.26, 0.03), mat("d0b879", 0.3))
	emblem.rotation.z = PI / 4
	box(node, Vector3(0, 0.8, -0.065), Vector3(0.045, 0.72, 0.035), mat("e2d3a2", 0.3))

static func knight(parent: Node3D, pos: Vector3, statue: bool = false) -> Node3D:
	var node = Node3D.new()
	node.position = pos
	parent.add_child(node)
	var armor = mat("737f83" if statue else "454e5d", 0.65)
	var edge = mat("aa9c77" if statue else "978568", 0.65)
	var dark = mat("28313b", 0.25)
	for side in [-1.0, 1.0]:
	...
	if not statue:
		for side in [-1.0, 1.0]:
			box(node, Vector3(side * 0.1, 2.01, -0.286), Vector3(0.1, 0.025, 0.03), mat("ff5e4f", 0, 2.0))
...
static func monster(parent: Node3D, kind: int) -> Node3D:
	...
	var armor = mat("3a3039", 0.3)
	var bone = mat("cbbb8c")
	var eyes = mat("ff7850", 0, 2.0)
	if kind == 1:
		...
		var blade = cylinder(node, Vector3(-0.9, 2.05, -0.34), 0.43, 0.43, 0.12, mat("879398", 0.7), 6)
...
static func weapon(parent: Node3D, tier: int) -> void:
	for child in parent.get_children():
		...
		child.queue_free()
	var wood = mat("765035")
	var steel = mat("a1b7bc", 0.75)
	var gold = mat("c9a76b", 0.6)
	# The armored hand is visible even before the first purchase.
	box(parent, Vector3(0, -0.07, 0.06), Vector3(0.18, 0.23, 0.23), mat("596978", 0.6))
